...
def get_berkeley_earth() -> xr.Dataset:
    ...
    temp = xr.Dataset(
        dict(
            tmax=xr.open_dataarray(datadir("processed", "berkeleyearth", "TMAX.nc")),
            tmin=xr.open_dataarray(
                datadir("processed", "processed", "berkeleyearth", "TMIN.nc")
            ),
            tavg=xr.open_dataarray(
                datadir("processed", "processed", "berkeleyearth", "TAVG.nc")
            ),
        )
    )
...
def get_ghcn_data(stnid: str) -> pd.DataFrame:
    ...
    fname = datadir("raw", "ghcnd_all", f"{stnid}.dly")
