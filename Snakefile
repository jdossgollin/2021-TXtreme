...
rule get_generators:
    output: datadir("raw", "eia", "november_generator2020.xlsx")
...
rule get_era5_conus_hourly:
    input: script = scriptdir("era5_get_conus_hourly.py")
    output: datadir("raw", "era5", "conus", "temp_hourly_{year}.nc")
    shell: "python {input.script} --year {wildcards.year} -o {output}"

# Download the raw berkeley earth temperature data
rule get_berkeley_earth:
    output: datadir("raw", "berkeleyearth", "{var}_{decade}.nc")
...
rule get_ghcnd_stations:
    output: datadir("raw", "ghcnd_stations.txt")
...
rule get_ghcnd_data:
    output: datadir("raw", "ghcnd_all", "USW00012960.dly")
...
rule era5_conus_anomalies:
    input:
        ...
        infiles = expand(datadir("raw", "era5", "conus", "temp_hourly_{year}.nc"), year=ERA_YEARS),
    output: datadir("processed", "era5", "conus", "daily_summaries.nc")
...
rule era5_tx_anomalies:
    input:
        script = scriptdir("era5_daily_summaries.py"),
        infiles = expand(datadir("raw", "era5", "tx", "temp_hourly_{year}.nc"), year=ERA_YEARS),
    output: datadir("processed", "era5", "tx", "daily_summaries.nc")
    shell: "python {input.script} -i {input.infiles} -o {output}"
...
rule era5_tx_hdd:
    input:
        ...
        temperature = expand(datadir("raw", "era5", "tx", "temp_hourly_{year}.nc"), year=ERA_YEARS),
        population = datadir("raw", "gpwv4", "gpw_v4_population_density_rev11_2pt5_min.nc")
    output: datadir("processed", "era5", "tx", "pop_hdd.nc")
...
rule hdd_ercot:
    ...
    output: datadir("processed", "era5", "tx", "hdd_ercot.nc")
...
rule era5_return:
    input:
        ...
        infiles = expand(datadir("raw", "era5", "tx", "temp_hourly_{year}.nc"), year=ERA_YEARS),
    output: datadir("processed", "era5", "tx", "return_period.nc")
    shell: "python {input.script} -i {input.infiles} -o {output}"
...
rule berkeley_earth_aggregate:
    input: 
        ...
        files = expand(datadir("raw", "berkeleyearth", "{{var}}_{decade}.nc"), decade=BERKELEY_DECADES),
    output: datadir("processed", "berkeleyearth", "{var}.nc")
...
rule berkeley_earth_hdd:
    ...
    output: datadir("processed", "berkeleyearth", "hdd.nc")
...
rule ghcn_subset:
    ...
    output: datadir("processed", "ghnd", "valid_stations.csv")
...
rule historical_exceedance:
    ...
    output: datadir("processed", "ghcnd", "return_periods.csv")
    shell: "python {input.script} -i {input.stations} -o {output}"
...
rule hdd_idf:
    ...
    output:
        ...
        figdir("ERCOT_HDD_IDF_plotpos_popweighted.pdf"),
        figdir("ERCOT_HDD_IDF_plotpos_unweighted.pdf"),
    notebook: scriptdir("hdd_idf.ipynb")
...
rule historic_extremes:
    ...
    output:
        ...
        figdir("historic_events_era5_TX.pdf"),
        figdir("historic_events_bk.pdf"),
    notebook: scriptdir("historic_extremes.ipynb")
...
rule local_return_period:
    ...
    output:
        figdir("local_rt_ghcnd.pdf"),
        figdir("local_rt_era5.pdf"),
    notebook: scriptdir("local_return_period.ipynb")

rule HDD_pop_weighted_ts:
    input: datadir("processed", "era5", "tx", "hdd_ercot.nc")
    output: figdir("HDD_pop_weighted_ts.pdf")
    notebook: scriptdir("time_series.ipynb")